...
import { TrailerEmbed } from "../../components/trailer-embed/TrailerEmbed";
...
describe('TrailerEmbed component', () => {
  ...
  it('renders the iframe element with correct className', () => {
    render(<TrailerEmbed link={mockLink} title={mockTitle} />);
    const iframeElement = screen.getByTitle(mockTitle);
    expect(iframeElement).toHaveClass('h-[65vh] w-full bg-gray-50 px-5 py-5 dark:bg-gray-900 lg:w-6/12');
  });

  it('renders the iframe element with correct allow attribute', () => {
    render(<TrailerEmbed link={mockLink} title={mockTitle} />);
    const iframeElement = screen.getByTitle(mockTitle);
    expect(iframeElement).toHaveAttribute('allow', 'accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture');
  });
