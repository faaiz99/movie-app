...
describe("Movie - Controller", () => {
	...
	afterAll(async () => {
		await request(app).delete(`/api/movies/${movieId}`).set("authorization", `Bearer ${token}`);
	...
	describe("Create Movie", () => {
		...
		it("POST api/movies should return 401 Unauthorized with invalid token", async () => {
			const response = await request(app).post("/api/movies").set("authorization", `Bearer ${token}1`).send(payload);
			expect(response.status).toBe(401);
			expect(response.body.status).toEqual("error");
			expect(response.body.message).toEqual("Invalid Token");
		});
		it("POST api/movies should return 200 OK with valid data", async () => {
			const response = await request(app).post("/api/movies").set("authorization", `Bearer ${token}`).send(payload);
		...
		it("POST api/movies should return 422 Unprocessable Content with incomplete data", async () => {
			...
			const response = await request(app).post("/api/movies").set("authorization", `Bearer ${token}`).send(modifiedPayload);
			expect(response.status).toBe(422);
		});

	});
	describe("Get Movie", () => {
		it("GET api/movies should return 403 Unauthorized with missing auth header", async () => {
	...
	describe("Update Movie", () => {
		it("POST api/movies/movieId should return 403 Unauthorized with missing auth header", async () => {
		...
		it("POST api/movies/movieId should return 200 Movie Updated with valid data", async () => {
			...
			const response = await request(app).post(`/api/movies/${movieId}`).set("authorization", `Bearer ${token}`).send(payload);
			expect(response.status).toBe(200);
		...
		it("POST api/movies/movieId should return 404 Movie Not Found with invalid MovieId", async () => {
			const modifiedMovieId = "8fcc29dd-4961-4be1-8cf0-ce179634a4ce";
			const response = await request(app).post(`/api/movies/${modifiedMovieId}`).set("authorization", `Bearer ${token}`).send(payload);
			expect(response.status).toBe(404);
			expect(response.body.message).toEqual("Movie Not Found");
		});
	...
	describe("Delete Movie", () => {
		it("DELETE api/movies/movieId should return 200 Movie Deleted with valid data", async () => {
			const response = await request(app).delete(`/api/movies/${movieId}`).set("authorization", `Bearer ${token}`);
			expect(response.status).toBe(200);
		});
		it("DELETE api/movies/movieId should return 422 Movie Not Deleted with missing MovieId", async () => {
			const modifiedMovieId = undefined;
			const response = await request(app).delete(`/api/movies/${modifiedMovieId}`).set("authorization", `Bearer ${token}`);
			expect(response.status).toBe(422);
			expect(response.body).toBeDefined();
			expect(response.body.length).toBeGreaterThan(0);
		});
